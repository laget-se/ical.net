...
namespace Ical.Net.Tests
{
    public class SimpleDeserializationTests
    {
        ...
        public void Attendee1()
        {
            ...
            Assert.Equal(1, iCal.Events.Count);
            ...
            // Values
            Assert.Equal(new Uri("mailto:joecool@example.com"), attendee1[0].Value);
            Assert.Equal(new Uri("mailto:ildoit@example.com"), attendee2.Value);

            // MEMBERS
            Assert.Equal(1, attendee1[0].Members.Count);
            Assert.Equal(0, attendee2.Members.Count);
            Assert.Equal(new Uri("mailto:DEV-GROUP@example.com"), attendee1[0].Members[0]);

            // DELEGATED-FROM
            Assert.Equal(0, attendee1[0].DelegatedFrom.Count);
            Assert.Equal(1, attendee2.DelegatedFrom.Count);
            Assert.Equal(new Uri("mailto:immud@example.com"), attendee2.DelegatedFrom[0]);

            // DELEGATED-TO
            Assert.Equal(0, attendee1[0].DelegatedTo.Count);
            Assert.Equal(0, attendee2.DelegatedTo.Count);
        }
        ...
        public void Attendee2()
        {
            ...
            // MEMBERS
            Assert.Equal(3, attendee1[0].Members.Count);
            Assert.Equal(new Uri("mailto:DEV-GROUP@example.com"), attendee1[0].Members[0]);
            Assert.Equal(new Uri("mailto:ANOTHER-GROUP@example.com"), attendee1[0].Members[1]);
            Assert.Equal(new Uri("mailto:THIRD-GROUP@example.com"), attendee1[0].Members[2]);
        }
        ...
        public void Bug2033495()
        {
            ...
            Assert.Equal(1, iCal.Events.Count);
            Assert.Equal(iCal.Properties["X-LOTUS-CHILD_UID"].Value, "XXX");
        }
        ...
        public void Bug2938007()
        {
            ...
            var evt = iCal.Events.First();
            Assert.Equal(true, evt.Start.HasTime);
            Assert.Equal(true, evt.End.HasTime);

            foreach (var o in evt.GetOccurrences(new CalDateTime(2010, 1, 17, 0, 0, 0), new CalDateTime(2010, 2, 1, 0, 0, 0)))
            {
                Assert.Equal(true, o.Period.StartTime.HasTime);
                Assert.Equal(true, o.Period.EndTime.HasTime);
            }
        }
        ...
        public void Categories1_2()
        {
            ...
            foreach (string item in items)
                Assert.True(found.ContainsKey(item), "Event should contain CATEGORY '" + item + "', but it was not found.");
        }
        ...
        public void EmptyLines1()
        {
            ...
            Assert.Equal(2, iCal.Events.Count, "iCalendar should have 2 events");
        }
        ...
        public void EmptyLines2()
        {
            ...
            Assert.Equal(2, calendars[0].Events.Count, "iCalendar should have 2 events");
            Assert.Equal(2, calendars[1].Events.Count, "iCalendar should have 2 events");
        }
        ...
        public void EmptyLines3()
        {
            ...
            Assert.Equal(1, iCal.Todos.Count, "iCalendar should have 1 todo");
        }
        ...
        public void EmptyLines4()
        {
            ...
            Assert.Equal(28, iCal.Events.Count);
        }
...
"This is a test to try out base64 encoding without being too large.",
        ...
        public void Encoding3()
        {
            ...
            Assert.Equal("uuid1153170430406", evt.Uid, "UID should be 'uuid1153170430406'; it is " + evt.Uid);
            Assert.Equal(1, evt.Sequence, "SEQUENCE should be 1; it is " + evt.Sequence);
        }
...
");
            ...
            Assert.True(iCal.Events.Count == 2, "There should be 2 events in the parsed calendar");
            Assert.NotNull(iCal.Events["fd940618-45e2-4d19-b118-37fd7a8e3906"], "Event fd940618-45e2-4d19-b118-37fd7a8e3906 should exist in the calendar");
            Assert.NotNull(iCal.Events["ebfbd3e3-cc1e-4a64-98eb-ced2598b3908"], "Event ebfbd3e3-cc1e-4a64-98eb-ced2598b3908 should exist in the calendar");
        }
        ...
        public void GeographicLocation1_2()
        {
            ...
            Assert.Equal(37.386013, evt.GeographicLocation.Latitude, "Latitude should be 37.386013; it is not.");
            Assert.Equal(-122.082932, evt.GeographicLocation.Longitude, "Longitude should be -122.082932; it is not.");
        }
        ...
        public void Google1()
        {
            ...
            for (var i = 0; i < dateTimes.Length; i++)
                Assert.Equal(dateTimes[i], occurrences[i].Period.StartTime, "Event should occur at " + dateTimes[i]);

            Assert.Equal(dateTimes.Length, occurrences.Count, "There should be exactly " + dateTimes.Length + " occurrences; there were " + occurrences.Count);
        }
        ...
        public void RecurrenceDates1()
        {
            ...
            Assert.Equal(1, iCal.Events.Count);
            Assert.Equal(3, iCal.Events.First().RecurrenceDates.Count);

            Assert.Equal((CalDateTime)new DateTime(1997, 7, 14, 12, 30, 0, DateTimeKind.Utc), iCal.Events.First().RecurrenceDates[0][0].StartTime);
            Assert.Equal((CalDateTime)new DateTime(1996, 4, 3, 2, 0, 0, DateTimeKind.Utc), iCal.Events.First().RecurrenceDates[1][0].StartTime);
            Assert.Equal((CalDateTime)new DateTime(1996, 4, 3, 4, 0, 0, DateTimeKind.Utc), iCal.Events.First().RecurrenceDates[1][0].EndTime);
            Assert.Equal(new CalDateTime(1997, 1, 1), iCal.Events.First().RecurrenceDates[2][0].StartTime);
            Assert.Equal(new CalDateTime(1997, 1, 20), iCal.Events.First().RecurrenceDates[2][1].StartTime);
            Assert.Equal(new CalDateTime(1997, 2, 17), iCal.Events.First().RecurrenceDates[2][2].StartTime);
            Assert.Equal(new CalDateTime(1997, 4, 21), iCal.Events.First().RecurrenceDates[2][3].StartTime);
            Assert.Equal(new CalDateTime(1997, 5, 26), iCal.Events.First().RecurrenceDates[2][4].StartTime);
            Assert.Equal(new CalDateTime(1997, 7, 4), iCal.Events.First().RecurrenceDates[2][5].StartTime);
            Assert.Equal(new CalDateTime(1997, 9, 1), iCal.Events.First().RecurrenceDates[2][6].StartTime);
            Assert.Equal(new CalDateTime(1997, 10, 14), iCal.Events.First().RecurrenceDates[2][7].StartTime);
            Assert.Equal(new CalDateTime(1997, 11, 28), iCal.Events.First().RecurrenceDates[2][8].StartTime);
            Assert.Equal(new CalDateTime(1997, 11, 29), iCal.Events.First().RecurrenceDates[2][9].StartTime);
            Assert.Equal(new CalDateTime(1997, 12, 25), iCal.Events.First().RecurrenceDates[2][10].StartTime);
        }
        ...
        public void RequestStatus1()
        {
            ...
            Assert.Equal(1, iCal.Events.Count);
            Assert.Equal(4, iCal.Events.First().RequestStatuses.Count);

            var rs = iCal.Events.First().RequestStatuses[0];
            Assert.Equal(2, rs.StatusCode.Primary);
            Assert.Equal(0, rs.StatusCode.Secondary);
            Assert.Equal("Success", rs.Description);
            Assert.IsNull(rs.ExtraData);

            rs = iCal.Events.First().RequestStatuses[1];
            Assert.Equal(3, rs.StatusCode.Primary);
            Assert.Equal(1, rs.StatusCode.Secondary);
            Assert.Equal("Invalid property value", rs.Description);
            Assert.Equal("DTSTART:96-Apr-01", rs.ExtraData);

            rs = iCal.Events.First().RequestStatuses[2];
            Assert.Equal(2, rs.StatusCode.Primary);
            Assert.Equal(8, rs.StatusCode.Secondary);
            Assert.Equal(" Success, repeating event ignored. Scheduled as a single event.", rs.Description);
            Assert.Equal("RRULE:FREQ=WEEKLY;INTERVAL=2", rs.ExtraData);

            rs = iCal.Events.First().RequestStatuses[3];
            Assert.Equal(4, rs.StatusCode.Primary);
            Assert.Equal(1, rs.StatusCode.Secondary);
            Assert.Equal("Event conflict. Date/time is busy.", rs.Description);
            Assert.IsNull(rs.ExtraData);
        }
        ...
        public void Transparency2()
        {
            ...
            Assert.Equal(TransparencyType.Transparent, evt.Transparency);
        }
        ...
        public void DateTime1()
        {
            ...
            Assert.Equal(6, iCal.Events.Count);
            ...
            Assert.Equal(DateTime.MinValue, evt.Created.Value);
        }

        [Fact(Skip = "Ignore until @thoemy commits the EventStatus.ics file"), Category("Deserialization")]
        public void EventStatus()
        {
            ...
            Assert.Equal(4, iCal.Events.Count);

            Assert.Equal("No status", iCal.Events[0].Summary);
            Assert.Null(iCal.Events[0].Status);
            Assert.True(iCal.Events[0].IsActive);

            Assert.Equal("Confirmed", iCal.Events[1].Summary);
            Assert.Equal("CONFIRMED", iCal.Events[1].Status);
            Assert.True(iCal.Events[1].IsActive);

            Assert.Equal("Cancelled", iCal.Events[2].Summary);
            Assert.Equal("CANCELLED", iCal.Events[2].Status);
            Assert.False(iCal.Events[2].IsActive);

            Assert.Equal("Tentative", iCal.Events[3].Summary);
            Assert.Equal("TENTATIVE", iCal.Events[3].Status);
            Assert.True(iCal.Events[3].IsActive);
        }
        ...
        public void Outlook2007_LineFolds2()
        {
            ...
            Assert.Equal(longName, ((CalendarEvent)events[0].Source).Location);
        }
        ...
        public void Parameter1()
        {
            ...
            Assert.Equal("DATE", parms[0].Values.First());
            Assert.Equal("OTHER", parms[1].Values.First());
        }
        ...
        public void Parameter2()
        {
            ...
            Assert.Equal(2, iCal.Events.Count);
        }
        ...
        public void Property1()
        {
            ...
            for (var i = 0; i < props.Count; i++)
                Assert.Equal("2." + i, props[i].Value);
        }
    }
}
