...
namespace Ical.Net.Tests
{
    ...
    public class DeserializationTests
    {
        ...
        public void Attendee1()
        {
            ...
            // Values
            Assert.Equal(new Uri("mailto:joecool@example.com"), attendee1.Value);
            Assert.Equal(new Uri("mailto:ildoit@example.com"), attendee2.Value);

            // MEMBERS
            Assert.Equal(1, attendee1.Members.Count);
            Assert.Equal(0, attendee2.Members.Count);
            Assert.Equal(new Uri("mailto:DEV-GROUP@example.com"), attendee1.Members[0]);

            // DELEGATED-FROM
            Assert.Equal(0, attendee1.DelegatedFrom.Count);
            Assert.Equal(1, attendee2.DelegatedFrom.Count);
            Assert.Equal(new Uri("mailto:immud@example.com"), attendee2.DelegatedFrom[0]);

            // DELEGATED-TO
            Assert.Equal(0, attendee1.DelegatedTo.Count);
...
";
        ...
        public void Parse1()
        {
            ...
            {
                Assert.IsType<SerializationException>(e);
            }
        }
